[
    {
        ...
        "data":[
            ...
            },
            {
                ...
                "rule":{
                    "首页规则":'var 列表=getVar("源码").replace(/\\s+/g,"").match(/\\{"lUid"[^\\{]+?sRoomName.+?sRecommendTagName.+?\\}/g);var 标题规则=".json(sRoomName)";var 地址规则=".c(/).json(lProfileRoom)";var 图片规则=".json(sScreenshot)";var 简介规则=".json(sIntroduction)";var NEXTPAGE="";var PREPAGE="";',
                    ...
                    "分类规则":'var 列表=e2Arr(getVar("源码").replace(/\\s+/g,""),".z(\\\\{.+).json(data).json(datas)");var 标题规则=".json(roomName)";var 地址规则=".c(/).json(profileRoom)";var 图片规则=".json(screenshot)";var 简介规则=".json(introduction)";var NEXTPAGE="";var PREPAGE="";',
                    "选集规则":'var 标识=e2Rex(getVar("源码"),".ty(HNF_GLOBAL_INIT)").match(/"sStreamName":"(.+?)"/)[1];if(标识.indexOf("imgplus")==-1){标识=标识+"-imgplus";}var 分类=[];var a={};var data=[];data.push({name:"P2p线路",url:"http://txtest-xp2p.p2p.huya.com/src/"+标识+".xs"},{name:"Hw.flv线路",url:"https://hw.flv.huya.com/src/"+标识+".m3u8"},{name:"tx.hls线路",url:"http://36.101.206.161/txdirect.hls.huya.com/src/"+标识+".m3u8"});a.data=data;a.title="线路";分类.push(a);var 分类=e2Arr(JSON.stringify(分类),".json()");var 线路="";var 简介=e2Rex(getVar("name"),".t()");var 列表规则=".json(data)";var 标题规则=".json(title)";var 选集规则=".json(name)";var 选集地址规则=".json(url)";',
                    "搜索规则":'var URL="https://search.cdn.huya.com/?m=Search&do=getSearchContent&q="+getVar("KEY")+"&uid=0&v=4&typ=-5&livestate=0&rows=40";var 源码=getHttp(URL);var 列表=e2Arr(源码,".json(response).json(3).json(docs)");var 标题规则=".json(game_roomName)";var 地址规则=".c(/).json(room_id)";var 图片规则=".json(game_screenshot)";var 简介规则=".json(game_introduction)";var NEXTPAGE="";var PREPAGE="";',
