        ...
        "data":[
            ...
            },
            {
                ...
                "rule":{
                    ...
                    "免嗅探规则":'var code=getHttp(getVar("url"));if(code.indexOf("url:")!=-1){var playurl=code.match(/url:.*?\'(.+?)\'/)[1];JSON.stringify({url:playurl+"?type=.mp4"});}else{var 播放线路=e2Rex(code,".get(div.dooplay_player li)");var u="https://www.4kvm.com/wp-json/dooplayer/v1/post/"+e2Rex(播放线路,".a(data-post)")+"?type="+e2Rex(播放线路,".a(data-type)")+"&source="+e2Rex(播放线路,".a(data-nume)");var uu=e2Rex(getHttp(u),".json(embed_url)");var resp=getHttp(uu);var playurl=resp.match(/url:.*?\'(.+?)\'/)[1];JSON.stringify({url:playurl+"?type=.mp4"});}'
                }
            }
        ]
    },
