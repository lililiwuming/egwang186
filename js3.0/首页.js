        ...
        "data":[
            ...
            },
            {
                ...
                "rule":{
                    ...
                    "选集规则":'var 多季=e2Rex(getVar("源码"),".get(div#episodes)");if(多季){var urls=[];var 总季=e2Arr(getVar("源码"),".get(div#episodes div.se-q)");for(var i in 总季){var name=e2Rex(总季[i],".get(span.title).t()");var url=e2Rex(总季[i],".get(a).a(href)");urls.push({name:name,url:url});}var 分类=[];var d=[];eval(getVar("QJS"));for(let index=0;index<urls.length;index++){function fn(i){return function(){var code=getHttp(urls[i].url);var 当前季集=JSON.parse(code.match(/videourls:\\[(.+)\\]/)[1]);var 接口=code.match(/ifsrc:\'(.+)\'/)[1];for(var k in 当前季集){当前季集[k].url=接口+"&source=0&ep="+当前季集[k].url;}return 当前季集};}d.push(fn(index));}var result=[];var s=_.submit(d,urls.length);for(let i=0;i<s.length;i++){for(let z of s[i].get()){result.push(z);}}for(var i in urls){分类.push({title:urls[i].name,list:JSON.stringify(result[i])});}var 简介=e2Rex(getVar("源码"),".get(div.wp-content)");var 线路="";var 列表规则=".json(list)";var 标题规则=".json(title)";var 选集规则=".json(name)";var 选集地址规则=".json(url)";}else{var 分类=[];var 当前季集=JSON.parse(getVar("源码").match(/videourls:\\[(.+)\\]/)[1]);var 接口=getVar("源码").match(/ifsrc:\'(.+)\'/)[1];for(var k in 当前季集){当前季集[k].url=接口+"&source=0&ep="+当前季集[k].url;}分类.push({title:"播放列表",list:当前季集});var 简介=e2Rex(getVar("源码"),".get(div.wp-content)");var 线路="";var 列表规则=".json(list)";var 标题规则=".json(title)";var 选集规则=".json(name)";var 选集地址规则=".json(url)";}',
