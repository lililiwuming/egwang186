        ...
        "data":[
            ...
            },
            {
                ...
                "rule":{
                    "首页规则":'var 列表=e2Arr(getVar("源码"),".get(ul.myui-vodlist li)");var 标题规则=".get(.title a).t()";var 地址规则=".get(.title a).a(href)";var 图片规则=".get(a).a(data-original)";var 简介规则=".get(span).st().t().c().get(p).t()";var 图片底部规则="";var 左上规则="";var 右上规则="";var NEXTPAGE="";var PREPAGE="";',
                    "筛选数据":'var a="分类+电影=1+电视剧=2+综艺=3+动漫=4+动作片=6+喜剧片=7+爱情片=8+科幻片=9+恐怖片=10+剧情片=11+国产剧=13+港台剧=14+日韩剧=15+欧美剧=16";var b="";for(var i=1;i<50;i++){b=b+"+第"+i+"页="+i;}var b="翻页+"+b;a+"\\n"+b;',
                    "分类规则":'alert("请等待几秒...列表加载中");var 列表=e2Arr(getVar("源码"),".get(ul.myui-vodlist li)");var 标题规则=".get(.title a).t()";var 地址规则=".get(.title a).a(href)";var 图片规则=".get(a).a(data-original)";var 简介规则=".get(span).st().t().c().get(p).t()";var 图片底部规则="";var 左上规则="";var 右上规则="";var NEXTPAGE="";var PREPAGE="";',
                    "选集规则":'var 分类=e2Arr(getVar("源码"),".get(ul[class~=.*?ui-content__.*?list])");var 线路=e2Arr(getVar("源码"),".get(ul.nav.nav-tabs li)");var 简介=e2Rex(getVar("源码"),".get(div[class~=.*?ui-content__detail]).t().c().get(div.col-pd.text-collapse.content).t()");var 列表规则=".get(li)";var 标题规则=".get(a).t()";var 选集规则=".get(a).t()";var 选集地址规则=".get(a).a(href)";',
            ...
            },
            {
                ...
                "baseURL":'"http://api.kunyu77.com/api.php/provide";',
                "rule":{
            ...
            },
            {
                ...
                "rule":{
                    ...
                    "选集规则":'var 多季=e2Rex(getVar("源码"),".get(div#episodes)");if(多季){var urls=[];var 总季=e2Arr(getVar("源码"),".get(div#episodes div.se-q)");for(var i in 总季){var name=e2Rex(总季[i],".get(span.title).t()");var url=e2Rex(总季[i],".get(a).a(href)");urls.push({name:name,url:url});}var 分类=[];var d=[];eval(getVar("QJS"));for(let index=0;index<urls.length;index++){function fn(i){return function(){if(urls[i].url){var code=getHttp(urls[i].url);var 当前季集=JSON.parse(code.match(/videourls:\\[(.+)\\]/)[1]);var 接口=code.match(/ifsrc:\'(.+)\'/)[1];for(var k in 当前季集){当前季集[k].url=接口+"&source=0&ep="+当前季集[k].url;}return 当前季集;}else{var 定位=e2Arr(getVar("源码"),".get(div#episodes div.se-a)");var 当前季集=[];var 集列表=e2Rex(定位[i],".get(li)");for(var m in 集列表){var name=e2Rex(集列表[m],".get(a).t()");var url=e2Rex(集列表[m],".get(a).a(href)");当前季集.push({name:name,url:url});}return 当前季集;}};}d.push(fn(index));}var result=[];var s=_.submit(d,urls.length);for(let i=0;i<s.length;i++){for(let z of s[i].get()){result.push(z);}}for(var i in urls){分类.push({title:urls[i].name,list:result[i]});}var 分类=e2Arr(JSON.stringify(分类),".json()");var 简介=e2Rex(getVar("源码"),".get(div.wp-content)");var 线路="";var 列表规则=".json(list)";var 标题规则=".json(title)";var 选集规则=".json(name)";var 选集地址规则=".json(url)";}else{var 分类=[];try{var 当前季集=JSON.parse(getVar("源码").match(/videourls:\\[(.+)\\]/)[1]);}catch(err){alert("此资源需付费,看看别的吧");}var 接口=getVar("源码").match(/ifsrc:\'(.+)\'/)[1];for(var k in 当前季集){当前季集[k].url=接口+"&source=0&ep="+当前季集[k].url;}分类.push({title:"播放列表",list:当前季集});var 分类=e2Arr(JSON.stringify(分类),".json()");var 简介=e2Rex(getVar("源码"),".get(div.wp-content)");var 线路="";var 列表规则=".json(list)";var 标题规则=".json(title)";var 选集规则=".json(name)";var 选集地址规则=".json(url)";}',
                    ...
                    "免嗅探规则":'var code=getHttp(getVar("url"));var playurl=code.match(/url:.*?\'(.+?)\'/)[1];JSON.stringify({url:playurl});'
                }
            }
        ]
    },
