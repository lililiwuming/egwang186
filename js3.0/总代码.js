...
var 模板库=[
    ...
    },
    {
        ...
        "rule":{
            ...
            "筛选数据":'var resp=getHttp(JSON.stringify({url:getVar("baseURL")+"/types",head:{"User-Agent":getVar("当前UA")}}));var 主=e2Arr(resp,".json(data).json(list)");var a="";for(var i in 主){a=a+"+"+JSON.parse(主[i]).type_name+"="+JSON.parse(主[i]).type_id+"&class=&area=";var 类型=e2Rex(主[i],".json(type_extend).json(class)").split(",").filter(Boolean);if(类型.length>1){for(var j in 类型){a=a+"+"+JSON.parse(主[i]).type_name+"-"+类型[j]+"="+JSON.parse(主[i]).type_id+"&class="+类型[j]+"&area=";}}var 地区=e2Rex(主[i],".json(type_extend).json(area)").split(",").filter(Boolean);if(地区.length>1){for(var k in 地区){a=a+"+"+JSON.parse(主[i]).type_name+"-"+地区[k]+"="+JSON.parse(主[i]).type_id+"&class=&area="+地区[k];}}}var a="分类"+a;var b="";for(var i=1;i<50;i++){b=b+"+第"+i+"页="+i;}var b="翻页+"+b;a+"\\n"+b;',
