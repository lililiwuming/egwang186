...
{"data":[{
    ...
    "rule":{
        ...
        "列表规则":'var 原始=JSON.parse(getCode().replace(/"(\\d+)"/g, \'"qq_$1"\')).data.initialAttributedText.text[0][0][0].c[1];var list=[];for(var key in 原始){if(原始[key].qq_2){list.push(原始[key].qq_2);}}alert(JSON.stringify(list));var 列表=[];for(var i=0;i<list.length;i=i+3){var title=list[i]?list[i][1]:""+list[i+1]?list[i+1][1]:"";var url=list[i]?list[i][1]:"";var detail=title+list[i+2]?list[i+2][1]:"";列表.push({title:title,url:url,detail:detail});}var 地址规则=".json(url)";var 标题规则=".json(url)";var 图片规则=".json(url)";var 简介规则=".json(detail)";var NEXTPAGE="";var PREPAGE="";',
        "详情规则":'var 正文=getVar("msg");'
    }
}
]
