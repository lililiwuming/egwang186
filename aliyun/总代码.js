...
if(getVar("url").indexOf("$$")!=-1){
    ...
    var code=getHttp(JSON.stringify({url:"https://api.aliyundrive.com/v2/file/get_share_link_download_url",head:{"Authorization":access_token,"X-Share-Token":share_token},postJson:JSON.stringify({share_id:share_id,file_id:file_id,expire_sec:0})}));
    }
...
alert("请重新登陆阿里云盘网页");
    ...
    JSON.stringify([{name:"原始文件播放",url:u,head:{"Referer":"https://www.aliyundrive.com/"}},{name:"转码m3u8可投屏",url:转码链接,head:{"Referer":"https://www.aliyundrive.com/"}}]);
}
######文档预览8
var cm=android.webkit.CookieManager.getInstance();
    var ALICOOKIE=cm.getCookie("www.aliyundrive.com");
